...
export PROJECT_VERSION=2.2.21
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=2.2.21
...
sed -i -E 's/(classpath "ru.vyarus:gradle-animalsniffer-plugin):1.2.0/\1:1.4.6/' build.gradle
...
if [ -z "2.2.21" ]; then
  echo "Enforce version not set, recreating original tag v2.2.21"
  git tag -m v2.2.21 -a v2.2.21 || true
else
  echo "Creating tag 2.2.21 to match enforced version"
  git tag -m 2.2.21 -a 2.2.21 || true
fi
